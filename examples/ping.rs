...
impl smoltcp::phy::RxToken for TockTapRxToken<'_> {
        ...
        F: FnOnce(&mut [u8]) -> smoltcp::Result<R>,
    {
        ...
        >(|handle| {
            ...
            let packet_length = self.rx_upcall.take().unwrap().0 as u16;
            writeln!(
                Console::writer(),
                "Received packet with length = {}",
                packet_length
            )
            .unwrap();
        ...
        if result.is_ok() {
            ...
            >(|handle| {
                ...
                TockSyscalls::command(DRIVER_NUM, SEND_PKT_CMD, len as u32, PACKET_IDENTIFIER)
                    ...
                    .unwrap();
                TockSyscalls::yield_wait();
                let packet_length = self.tx_upcall.take().unwrap().0 as u16;
                writeln!(
                    Console::writer(),
                    "Sent packet with length = {}",
                    packet_length
                )
                .unwrap();
                // For some reasons, this command returns ErrorCode::ALREADY
...
impl<'a> smoltcp::phy::Device<'a> for TockTapDevice {
    ...
    fn capabilities(&self) -> smoltcp::phy::DeviceCapabilities {
        let mut checksum_capabilities = smoltcp::phy::ChecksumCapabilities::ignored();
        checksum_capabilities.ipv4 = smoltcp::phy::Checksum::Both;
        checksum_capabilities.icmpv4 = smoltcp::phy::Checksum::Both;
...
fn main() {
    ...
    let icmp_rx_buffer =
        smoltcp::socket::IcmpSocketBuffer::new(&mut icmp_rx_metadata[..], &mut icmp_rx_payload[..]);

    let mut icmp_tx_metadata = [smoltcp::socket::IcmpPacketMetadata::EMPTY; 1];
    let mut icmp_tx_payload = [0 as u8; ETH_MTU];
    let icmp_tx_buffer =
        smoltcp::socket::IcmpSocketBuffer::new(&mut icmp_tx_metadata[..], &mut icmp_tx_payload[..]);

    let identifier = 1234;
    let mut icmp_socket = smoltcp::socket::IcmpSocket::new(icmp_rx_buffer, icmp_tx_buffer);
    icmp_socket
        .bind(smoltcp::socket::IcmpEndpoint::Ident(identifier))
        .unwrap();
    ...
    loop {
        ...
        interface.poll(timestamp).unwrap();
    }
}
